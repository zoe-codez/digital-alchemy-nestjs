...
export class MenuSampler {
  ...
  public async exec(value?: string): Promise<void> {
    ...
    const action = await this.prompt.menu({
      ...
      right: [
        ...
        {
          entry: ["advanced workflow", "advanced"],
        ...
        {
          entry: ["pretty shortcuts", "pretty"],
          helpText: "Showing different methods of coloring keyboard shortcuts",
        },
      ],
      value,
    });
    switch (action) {
      case "done":
        return;
      case "configurable":
        await this.configurable();
        return await this.exec(action);
      case "async":
        await this.async();
        return await this.exec(action);
      case "pretty":
        await this.prettyShortcuts();
        return await this.exec(action);
      case "advanced":
        this.storedValues = [];
        this.hiddenTypes = [];
  ...
  private async advanced(value?: AdvancedMenuResult): Promise<void> {
  ...
  private async configurable(): Promise<void> {
  ...
  private async prettyShortcuts(): Promise<void> {
